    ...
    public class TaskService : BaseService
    {
        ...
        public List<Task> GetTasks(string fromDate, string toDate, string userName, string location,string project,string workcode,string employee)
        {
            ...
            if (!string.IsNullOrEmpty(userName))
            {
                ...
                {
                    interResult.AddRange(tasks.Where(x => x.Employee.Login.UserId == val).ToList());
                }
                tasks = interResult;
            }
        ...
        public BillingModel GetBilling(string fromDate, string toDate, string userName, string location, string Company = "")
        {
            ...
            if (!string.IsNullOrEmpty(fromDate) && string.IsNullOrEmpty(toDate))
            {
                ...
                {
                    if (location == null)
                        tasks = this.db.Tasks.Include("Project").Include("Employee").Include("Login").Include("WorkCode").Where(x => x.ExecutionDate == from && x.Employee.Login.UserId.Equals(userName)).ToList();
                    else
                        tasks = this.db.Tasks.Include("Project").Include("Employee").Include("Login").Include("WorkCode").Where(x => x.ExecutionDate == from && x.Employee.Login.UserId.Equals(userName) && x.Employee.Location.Name.Equals(location)).ToList();
                }
            }
            else
            {
                ...
                {
                    if (location == null)
                        tasks = this.db.Tasks.Include("Project").Include("Employee").Include("WorkCode").Where(x => x.ExecutionDate >= from && x.ExecutionDate <= to && x.Employee.Login.UserId.Equals(userName)).OrderBy(x => x.ExecutionDate).ToList();
                    else
                        tasks = this.db.Tasks.Include("Project").Include("Employee").Include("WorkCode").Where(x => x.ExecutionDate >= from && x.ExecutionDate <= to && x.Employee.Login.UserId.Equals(userName) && x.Employee.Location.Name.Equals(location)).OrderBy(x => x.ExecutionDate).ToList();
                }
            }
            ...
            tasks.ForEach(x =>
            {
                ...
                if (workSummary == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(Company))
                    {
                        workSummary.PayableAmount = (x.Employee.Cost.HasValue ? x.Employee.Cost.Value * x.Hours : 0);
                        workSummary.Costperhour = (x.Employee.Cost.HasValue ? x.Employee.Cost.Value : 0);
                    }
                    workSummary.BillableAmount = (x.Employee.BillRate.HasValue ? x.Employee.BillRate.Value * x.Hours : 0);
                    workSummary.Company = x.Employee.CompanyName;                    
                    workSummary.Hours = x.Hours;
                    workSummary.Total = x.Charge;
        ...
        public BillingModel GetMTSBilling(string fromDate, string toDate, string userName, string location)
        {
            ...
            tasks.ForEach(x =>
            {
                ...
                if (workSummary == null)
                {
                    ...
                    workSummary.Total = x.Charge;                   
                    workSummary.Costperhour = (x.Employee.Cost.HasValue ? x.Employee.Cost.Value : 0);
                    workSummary.TotalCost = (x.Employee.Cost.HasValue ? x.Employee.Cost.Value * x.Hours : 0);
                    workSummary.BillableAmount = (x.Employee.BillRate.HasValue ? x.Employee.BillRate.Value * x.Hours : 0);
                    workSummary.PayableAmount = (x.Employee.Cost.HasValue ? x.Employee.Cost.Value * x.Hours : 0);
